...
For =latex= and =dvisvgm= paths on MacOS, I need to specify the full path of the program. Otherwise, even the =(executable-find "latex")= shows correctly, the preview subprocess still won't find the =latex=.

Using tecosaur's org-mode fork with the new =org-latex-preview= system.
...
#+begin_src emacs-lisp
  ...
  ;; New org-latex-preview appearance options (replaces org-format-latex-options)
  ...
  ;; Use dvisvgm as the default process
  (setq org-latex-preview-process-default 'dvisvgm)

  ;; Explicitly set LaTeX executable paths for org-latex-preview
  (let ((texlive-bin "/usr/local/texlive/2025/bin/universal-darwin"))
    ;; Set compiler command map with full paths - this controls what %l and %L expand to
    (setq org-latex-preview-compiler-command-map
          `(("pdflatex" . ,(concat texlive-bin "/latex"))
            ("xelatex" . ,(concat texlive-bin "/xelatex -no-pdf"))
            ("lualatex" . ,(concat texlive-bin "/dvilualatex"))))
    (setq org-latex-preview-process-alist
          `((dvisvgm
             ...
             :latex-compiler (,(concat texlive-bin "/latex -interaction nonstopmode -output-directory %o %f"))
             :latex-precompiler (,(concat texlive-bin "/latex -output-directory %o -ini -jobname=%b \"&%L\" mylatexformat.ltx %f"))
             :image-converter (,(concat texlive-bin "/dvisvgm --page=1- --optimize --clipjoin --relative --no-fonts --bbox=preview -o %B-%%9p.svg %f"))))))

  ;; Enable persistent cache across Emacs sessions
  (setq org-latex-preview-cache 'persist)

  ;; Cache expiry in days
  (setq org-latex-preview-persist-expiry 7)

  ;; Enable equation numbering
  (setq org-latex-preview-numbered t)

  ;; Show processing indicator in fringe
  (setq org-latex-preview-process-active-indicator 'fringe)

  ;; Enable preamble precompilation for faster previews
  (setq org-latex-preview-process-precompile t)

  ;; Live preview while editing
  (setq org-latex-preview-mode-display-live '(block edit-special))

  ;; Debounce delay for live updates (seconds)
  (setq org-latex-preview-mode-update-delay 0.5)

  ;; Track fragment insertions for auto-preview
  ...
  ;; Enable org-latex-preview-mode in org buffers
  ...
  (use-package org-agenda
    ...
    (setq org-agenda-custom-commands
          '(("d" "Daily agenda and Ideas"
             ((todo "RESEARCH"
                    ((org-agenda-overriding-header "Scientific Projects:")))
              (todo "IDEA"
    ...
    (setq org-capture-templates '(("i" "Idea"
                                  ...
                                  ("s" "Scientific Project"
                                   entry (file+headline "~/Documents/gtd/projects.org" "Scientific")
                                   "* RESEARCH %?\nAdded: %U\n"
                                   :prepend t
                                   :kill-buffer t))))
#+end_src
